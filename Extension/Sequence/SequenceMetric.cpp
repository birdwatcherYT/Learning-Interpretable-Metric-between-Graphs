...
SequenceMetric::SequenceMetric(const std::string& filename, int minsup, int maxpat, double train, int Ksample) {
    this->minsup = minsup;
    this->maxpat = maxpat;
...
void SequenceMetric::setKsample(const string &kernel){
...
void SequenceMetric::nearest(const std::string &mat){
    ifstream ifs(mat);
    if (ifs.fail()){
...
tree<SequenceMetric::Save>::iterator SequenceMetric::createRoot(){
    if (Tree.empty()){
        ProjectDB pdb;
        for (int i = 0; i < trainSize; ++i)
...
void SequenceMetric::createChildren(const tree<Save>::iterator &node){
    if (!node->nextCheck){
        node->nextCheck=true;
        if (node->pattern.size() >= maxpat)
            return;
        map<int, ProjectDB> counter;
        if (node->pattern.size()==0){
...
double SequenceMetric::getMaxValue(const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD)  {
    visit=0;
    double maxval = 0;
...
void SequenceMetric::__getMaxValue(const tree<Save>::iterator &node, double& maxval, const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD){
    ++visit;
...
void SequenceMetric::safePatternPruning(double lambda_prev, double lambda, const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD, double z2, double epsilon_d, vecVec& Xt, PatternList& patternList) {
    clock_t start=clock();
    Xt.clear();
    patternList.clear();
    visit=0;
...
void SequenceMetric::__safePatternPruning(double lambda_prev, double lambda, const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD, double z2, double epsilon_d, const tree<Save>::iterator &node, vecVec& Xt, PatternList& patternList) {
...
double SequenceMetric::workingSetPruning(double lambda, const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD, vecVec& Xt, PatternList& patternList) {
    clock_t start=clock();
    Xt.clear();
    patternList.clear();
    visit=0;
...
double SequenceMetric::__workingSetPruning(double lambda, const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD, const tree<Save>::iterator &node, vecVec& Xt, PatternList& patternList) {
...
void SequenceMetric::regularizationPath(double L, double U, double eta, int splitNum, int maxloop, double eps, int freq, double R, const std::string &kernel) {
    setKsample(kernel);
...
vecVec SequenceMetric::getTestX(const PatternList& patternList) {
    ProjectDB pdb;
    for (int i = 0; i < testSize; ++i)
        pdb.push_back(pair<int, int>(i, -1));
...
void SequenceMetric::__getTestX(const ProjectDB& pdb, vecVec& Xt, Pattern& pattern, const PatternList& patternList) {
...
double SequenceMetric::error(const std::vector< std::vector<double> > &distance, const int K, int from, int to) const{
    double err=0;
    #pragma omp parallel 
    {
...
// distance : test x train
double SequenceMetric::f_measure(const std::vector< std::vector<double> > &distance, const int K, int from, int to, bool macro) const{
...
void SequenceMetric::run(const vecVec &active_Xt, const vecVec &test_Xt, const vector<double>& m, double lambda, double L, double U, double eta, double eps, unsigned loopMax) {
...
double SequenceMetric::kernel(const std::string &mat){
    ifstream ifs(mat);
    if (ifs.fail()){
        cerr<<"file open error"<<endl;
        exit(1);
