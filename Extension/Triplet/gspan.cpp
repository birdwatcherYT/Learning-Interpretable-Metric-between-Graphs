...
namespace GSPAN {
    ...
    double gSpan::getMaxValue(const std::vector< std::vector< std::vector<double> > >& zD) {
        visit=0;
        double maxval = 0;
        /////////////////////////////////////////////////////////////
        for (const Save &node : singleTree) {
            ...
            for (int i = 0; i < TRAIN.size(); ++i) {
                for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                    ...
                    for (int b=0,l=diffClass[i][b];b<Ksample;++b,l=diffClass[i][b]){
                        bool cil = (node.x[i] != node.x[l]);
                        dot += (cil - cij)*zD[i][a][b];
                    }
                }
            }
        ...
        for (tree<Save>::sibling_iterator it=Tree.begin(root);it!=Tree.end(root);++it)
            __getMaxValue(it, maxval, zD);

        cout<<"getMaxValue:visit = "<<visit<<endl;
        return maxval;
    }

    void gSpan::__getMaxValue(const tree<Save>::iterator &node, double& maxval, const std::vector< std::vector< std::vector<double> > >& zD) {
        ++visit;
        ...
        for (int i = 0; i < TRAIN.size(); ++i) {
            for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                ...
                for (int b=0,l=diffClass[i][b];b<Ksample;++b,l=diffClass[i][b]){
                    bool cil = (node->x[i] != node->x[l]);
                    dot += (cil - cij)*zD[i][a][b];
                    dotSPPC += fmax((double)node->x[i], (double)node->x[l])*zD[i][a][b];
                }
            }
        }
    ...
    void gSpan::safePatternPruning(double lambda_prev, double lambda, const std::vector< std::vector< std::vector<double> > >& zD, double z2, double epsilon_d, vecVec& Xt, std::vector<DFSCode>& patternList) {
        clock_t start=clock();
        Xt.clear();
        patternList.clear();
        visit=0;
        ...
        for (Save &node : singleTree) {
            ...
            if (node.screeningScore > lambda && dot > lambda) {
                Xt.push_back(node.x);
        ...
        for (tree<Save>::sibling_iterator it=Tree.begin(root);it!=Tree.end(root);++it)
            __safePatternPruning(lambda_prev, lambda, zD, z2, epsilon_d, it, Xt, patternList);
        clock_t end=clock();
        cout<<"safePatternPruning:visit = "<<visit<<endl;
        cout<<"safePatternPruning:time = "<<(double)(end-start)/CLOCKS_PER_SEC<<endl;
    }

    void gSpan::__safePatternPruning(double lambda_prev, double lambda, const std::vector< std::vector< std::vector<double> > >& zD, double z2, double epsilon_d, const tree<Save>::iterator &node, vecVec& Xt, std::vector<DFSCode>& patternList) {
            ...
            for (int i = 0; i < trainSize; ++i) {
                for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                    ...
                    for (int b=0,l=diffClass[i][b];b<Ksample;++b,l=diffClass[i][b]){
                        ...
                        dotSPPC += fmax((double)node->x[i], (double)node->x[l])*zD[i][a][b];
                        h_norm+=fabs(cil-cij);
                        h_normSPPC += fmax((double)node->x[i], (double)node->x[l]);
                    }
                }
            }
    ...
    double gSpan::workingSetPruning(double lambda, const std::vector< std::vector< std::vector<double> > >& zD, vecVec& Xt, std::vector<DFSCode>& patternList) {
        clock_t start=clock();
        Xt.clear();
        patternList.clear();
        visit=0;
        ...
        for (tree<Save>::sibling_iterator it=Tree.begin(root);it!=Tree.end(root);++it)
            m_alpha_lambda_eta2+=__workingSetPruning(lambda, zD, it, Xt, patternList);
        clock_t end=clock();
    ...
    double gSpan::__workingSetPruning(double lambda, const std::vector< std::vector< std::vector<double> > >& zD, const tree<Save>::iterator &node, vecVec& Xt, std::vector<DFSCode>& patternList) {
    ...
    void gSpan::regularizationPath(double L, double eta, int splitNum, int maxloop, double eps, int freq, double R, const string &kernel) {
        setKsample(kernel);
        ...
        double zD1=0, z2=0;
        double loss = 0;
        vector< vector< vector<double> >> zD(trainSize, vector< vector<double> >(Ksample, vector<double>(Ksample, 2*L)));
        zD1=2*L*Ksample*Ksample*trainSize;
        z2=2*L*2*L*Ksample*Ksample*trainSize;
        loss=L*L*Ksample*Ksample*trainSize;

        double lambda_prev = getMaxValue(zD); //LAMBDA_MAX
        ...
        vector< vector< vector<double> > > zDMaxWS=zD;
        double z2MaxWS=z2, zD1MaxWS=zD1;
        vector< vector< vector<double> > > zDMax=zD;
        double z2Max=z2;
        ...
        for (int idx = 1; idx < splitNum; ++idx) {
            ...
            safePatternPruning(lambda_prev, lambda, zDMax, z2Max, epsilon_d, Xt, pat);
            m.clear();
            ...
            for (int loop = 0; loop < maxloop; ++loop) {
                ...
                for (int i = 0; i < trainSize; ++i) {
                    for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                        for (int b=0, l=diffClass[i][b];b<Ksample;++b, l=diffClass[i][b]){
                            ...
                            double value = 0;
                            for (size_t k : index) {
                                cijl[k] = fabs(Xt[k][i] - Xt[k][l]) - fabs(Xt[k][i] - Xt[k][j]);
                                value += cijl[k]*m[k], h_norm[k]+=fabs(cijl[k]);
                            }
                            double hinge = fmax(0,L-value);
                            for (size_t k : index)
                                h[k] += cijl[k]*2*hinge;
                            loss += hinge*hinge, zD1 += 2*hinge, z2 += (2*hinge)*(2*hinge), zD[i][a][b] = 2*hinge;
                        }
                    }
                }
                for (size_t k : index)
                ...
                double dual = -0.25 * z2 + L*zD1-0.5*m_alpha2/(lambda*eta);

                if (dualMaxWS<dual)
                    dualMaxWS = dual, hMaxWS=h, zDMaxWS=zD, z2MaxWS=z2, zD1MaxWS=zD1;
                ...
                if (gap <= eps * primal) {// WS内での収束
                    ...
                    if (!(gap <= eps * primal)){// これまでのDualの値で収束しなかったとき
                        m_alpha2=workingSetPruning(lambda, zDMaxWS, Xt_temp, pat_temp);
                        dual = -0.25 * z2MaxWS + L*zD1MaxWS-0.5*m_alpha2/(lambda*eta);
                        if (dualMax<dual)
                            dualMax = dual, zDMax=zDMaxWS, z2Max=z2MaxWS;
                        gap = primal - dualMax;
                    }
