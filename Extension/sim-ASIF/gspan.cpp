...
namespace GSPAN {
    ...
    std::istream &gSpan::read(std::istream &is, double train, bool ignoreEdgeLabel, int VertexLabelDegreeQuantization) {
        ...
        computeVertexLabelDistance(temp);
        return is;
    }
    
    void gSpan::computeVertexLabelDistance(const std::vector <Graph> &database){
        for(const Graph &g : database){
            for (const Vertex &v : g){
                if ( vertexLabel2idx.find(v.label) == vertexLabel2idx.end() )
                    vertexLabel2idx[v.label] = vertexLabel2idx.size();
            }
        }

        int vertexLabelNum=vertexLabel2idx.size();
        std::vector< std::vector<double> > CoOccurrence;
        CoOccurrence.assign(vertexLabelNum, vector<double>(vertexLabelNum, 0));

        for(const Graph &g : database){
            for (const Vertex &v : g){
                for (const Edge &e : v.edge){
                    int fromLabel=g[e.from].label, toLabel=g[e.to].label;
                    CoOccurrence[vertexLabel2idx[fromLabel]][vertexLabel2idx[toLabel]] += 1;
                }
            }
        }
        ...
        for(int i=0;i<vertexLabelNum;++i){
            double norm=0;
            for(int j=0;j<vertexLabelNum;++j)
                norm+=CoOccurrence[i][j]*CoOccurrence[i][j];
            norm=sqrt(norm);
            for(int j=0;j<vertexLabelNum;++j)
                CoOccurrence[i][j]=CoOccurrence[i][j]/norm;
        }

        vertexLabelDistance.assign(vertexLabelNum, vector<double>(vertexLabelNum, 0));
        for(int i=0;i<vertexLabelNum;++i){
            for(int j=0;j<vertexLabelNum;++j){
                double dist=0;
                for(int k=0;k<vertexLabelNum;++k)
                    dist+=(CoOccurrence[i][k]-CoOccurrence[j][k])*(CoOccurrence[i][k]-CoOccurrence[j][k]);
                vertexLabelDistance[i][j]=sqrt(dist);
            }
        }
        //////////////////////////////////
        cout<<vertexLabelDistance<<endl;
        //////////////////////////////////
    }
                ...
                for (Projected_iterator2 elabel = fromlabel->second.begin(); elabel != fromlabel->second.end(); ++elabel) {
                    for (Projected_iterator1 tolabel = elabel->second.begin(); tolabel != elabel->second.end(); ++tolabel) {
                        ...
                        if (sup >= minsup && DFS_CODE.nodeCount() <= maxpat && is_min()){
                            ...
                            wl.updateWLlabeling(WLloop);
                            for(int i=0;i<trainSize;++i)
                                x[i]=exp(-featureScale*trainWL[i].contains(wl, vertexLabelDistance, vertexLabel2idx, distanceThreshold));
    ...
    void gSpan::__getMaxValue(const tree<Save>::iterator &node, double& maxval, const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD) {
        ...
        for (int i = 0; i < TRAIN.size(); ++i) {
            ...
            for (int a=0,l=diffClass[i][a];a<Ksample;++a,l=diffClass[i][a]){
                ...
                dot += cil*zD[i][a];
                dotSPPC += fmax(node->x[i]*node->x[i], node->x[l]*node->x[l]) * zD[i][a];
            }
        }
        ...
        if (node->pruningScore<=lambda){
            ...
            for (int i = 0; i < trainSize; ++i) {
                for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                    ...
                    h_normSPPC+=fmax(node->x[i]*node->x[i]*node->x[i]*node->x[i], node->x[j]*node->x[j]*node->x[j]*node->x[j]);
                }
                for (int a=0,l=diffClass[i][a];a<Ksample;++a,l=diffClass[i][a]){
                    double cil = (node->x[i] - node->x[l])*(node->x[i] - node->x[l]);
                    dot += cil*zD[i][a], h_norm+=cil*cil;
                    dotSPPC += fmax(node->x[i]*node->x[i], node->x[l]*node->x[l]) * zD[i][a];
                    h_normSPPC+=fmax(node->x[i]*node->x[i]*node->x[i]*node->x[i], node->x[l]*node->x[l]*node->x[l]*node->x[l]);
                }
            }
    ...
    void gSpan::regularizationPath(double L, double U, double eta, int splitNum, int maxloop, double eps, int freq, double R, const string &kernel) {
        ...
        cout<<"featureScale = "<<featureScale<<", featureThreshold = "<<featureThreshold<<", distanceThreshold = "<<distanceThreshold<<endl;
    ...
    vecVec gSpan::getTestX(const std::vector< DFSCode >& dfscodes) {
        ...
        for(DFSCode dfs : dfscodes){
            ...
            WL wl(g);
            wl.updateWLlabeling(WLloop);
            vector<double> x(testSize,0);
            for(int i=0;i<testSize;++i)
                x[i]=exp(-featureScale*testWL[i].contains(wl, vertexLabelDistance, vertexLabel2idx, distanceThreshold));
            Xt.push_back(x);
        }
        return Xt;
    }
