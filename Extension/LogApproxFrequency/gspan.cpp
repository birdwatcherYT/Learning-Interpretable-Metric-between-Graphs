...
namespace GSPAN {
            ...
            for (Projected_iterator3 fromlabel = Root.begin(); fromlabel != Root.end(); ++fromlabel) {
                for (Projected_iterator2 elabel = fromlabel->second.begin(); elabel != fromlabel->second.end(); ++elabel) {
                    for (Projected_iterator1 tolabel = elabel->second.begin(); tolabel != elabel->second.end(); ++tolabel) {
                        ...
                        // x=subgraph_counts(*projected, trainSize);
                        x=approximate_subgraph_counts(*projected, trainSize);
                        for (double &xi:x)
                            xi=log(1+xi);
    ...
    int gSpan::__subgraph_counts(const std::vector< std::set<int> > &nodeIDs, int begin, std::set<int> current, int depth)const{
        int max_depth=depth;
        for (int i=begin; i < nodeIDs.size(); ++i){
            set<int> unionset=current;
            bool duplicate=false;
            for (int id : nodeIDs[i]){// union計算
                size_t size=unionset.size();
                unionset.insert(id);
                if (unionset.size()==size){ // 重複あるなら次へ
                    duplicate=true;
                    break;
                }
            }
            if (duplicate)// pruning
                continue;
            if (max_depth>=depth+1+nodeIDs.size()-i-1)//改善の見込みがなければ
                continue;
            max_depth = max(max_depth, __subgraph_counts(nodeIDs, i+1, unionset, depth+1));
        }
        return max_depth;
    }
    
    vector< double > gSpan::subgraph_counts(const Projected &projected, size_t vector_size)const{
        map<int, vector< set<int> > > subgraph_nodes;// sampleID -> vector< nodeIDs >
        for (Projected::const_iterator cur = projected.begin(); cur != projected.end(); ++cur){
            set<int> nodeIDs;
            nodeIDs.insert(cur->edge->from);
            nodeIDs.insert(cur->edge->to);
            for (PDFS* p=cur->prev;p!=NULL;p=p->prev){
                nodeIDs.insert(p->edge->from);
                nodeIDs.insert(p->edge->to);
            }
            subgraph_nodes[cur->id].push_back(nodeIDs);
        }
        vector< double > x(vector_size, 0);
        for (auto &[key, value] : subgraph_nodes)
            x[key]=__subgraph_counts(value, 0, std::set<int>(), 0);
        return x;
    }
    
    std::vector< double > gSpan::approximate_subgraph_counts(const Projected &projected, size_t vector_size)const{
        vector<double> x(vector_size, 0);
        map<size_t, set< pair<int,int> > > edges;// sampleID->edges
        for (Projected::const_iterator cur = projected.begin(); cur != projected.end(); ++cur){
            int from=cur->edge->from, to=cur->edge->to;
            for (PDFS* p=cur->prev;p!=NULL;p=p->prev){
                from=p->edge->from;
                to=p->edge->to;
            }
            if (from<=to)
                edges[cur->id].insert(pair<int,int>(from, to));
            else
                edges[cur->id].insert(pair<int,int>(to, from));
        }
        ...
        for (const Save &node : singleTree) {
            ...
            for (int i = 0; i < TRAIN.size(); ++i) {
                for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                    double cij = (node.x[i] - node.x[j])*(node.x[i] - node.x[j]);
                    dot -= cij*zS[i][a];
                }
                for (int a=0,l=diffClass[i][a];a<Ksample;++a,l=diffClass[i][a]){
                    double cil = (node.x[i] - node.x[l])*(node.x[i] - node.x[l]);
                    dot += cil*zD[i][a];
                }
            }
    ...
    void gSpan::__getMaxValue(const tree<Save>::iterator &node, double& maxval, const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD) {
        ...
        for (int i = 0; i < TRAIN.size(); ++i) {
            ...
            for (int a=0,l=diffClass[i][a];a<Ksample;++a,l=diffClass[i][a]){
                double cil = (node->x[i] - node->x[l])*(node->x[i] - node->x[l]);
                dot += cil*zD[i][a];
                double max_x=fmax(node->x[i], node->x[l]);
                dotSPPC += max_x*max_x*zD[i][a];
            }
        }
    ...
    void gSpan::safePatternPruning(double lambda_prev, double lambda, const std::vector< std::vector<double> >& zS, const std::vector< std::vector<double> >& zD, double z2, double epsilon_d, vecVec& Xt, std::vector<DFSCode>& patternList) {
        ...
        for (Save &node : singleTree) {
            ...
            for (int i = 0; i < TRAIN.size(); ++i) {
                for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                    double cij = (node.x[i] - node.x[j])*(node.x[i] - node.x[j]);
                    dot -= cij*zS[i][a], h_norm+=cij*cij;
                }
                for (int a=0,l=diffClass[i][a];a<Ksample;++a,l=diffClass[i][a]){
                    double cil = (node.x[i] - node.x[l])*(node.x[i] - node.x[l]);
                    dot += cil*zD[i][a], h_norm+=cil*cil;
                }
            }
        ...
        if (node->pruningScore<=lambda){
            ...
            for (int i = 0; i < trainSize; ++i) {
                for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                    double cij = (node->x[i] - node->x[j])*(node->x[i] - node->x[j]);
                    dot -= cij*zS[i][a], h_norm+=cij*cij;
                    double max_x=fmax(node->x[i], node->x[j]);
                    h_normSPPC+=max_x*max_x*max_x*max_x;
                }
                for (int a=0,l=diffClass[i][a];a<Ksample;++a,l=diffClass[i][a]){
                    double cil = (node->x[i] - node->x[l])*(node->x[i] - node->x[l]);
                    dot += cil*zD[i][a], h_norm+=cil*cil;
    ...
    void gSpan::regularizationPath(double L, double U, double eta, int splitNum, int maxloop, double eps, int freq, double R, const string &kernel) {
        ...
        for (int idx = 1; idx < splitNum; ++idx) {
            ...
            for (int loop = 0; loop < maxloop; ++loop) {
                ...
                for (int i = 0; i < trainSize; ++i) {
                    for (int a=0,j=sameClass[i][a];a<Ksample;++a,j=sameClass[i][a]){
                        vector<double> cij(dim);
                        double value = 0;
                        for (size_t k : index) {
                            cij[k] = (Xt[k][i] - Xt[k][j])*(Xt[k][i] - Xt[k][j]);
                            value += cij[k]*m[k], h_norm[k]+=cij[k]*cij[k];
                        }
                    ...
                    for (int a=0, l=diffClass[i][a];a<Ksample;++a, l=diffClass[i][a]){
                        vector<double> cil(dim);
                        double value = 0;
                        for (size_t k : index) {
                            cil[k] = (Xt[k][i] - Xt[k][l])*(Xt[k][i] - Xt[k][l]);
                            value += cil[k]*m[k], h_norm[k]+=cil[k]*cil[k];
                        }
                    ...
                    for (int i=0;i<testSize;++i){
                        ...
                        for (int j=0;j<trainSize;++j)
                            for (int k=0;k<activeSize;++k)
                                distance[i][j] += active_m[k]*(testXt[k][i] - active_Xt[k][j])*(testXt[k][i] - active_Xt[k][j]);
                    }
            ...
            for (int i=Xt.size();i<dfscodes.size();++i ){
                if (dfscodes[i].contain(DFS_CODE) == 0){
                    for (int j=0, count=i - Xt.size(); j < count;++j )
                        Xt.push_back(vector<double>(TEST.size(), 0));
                    vector<double> x(TEST.size(),0);
        ...
        for (int i=Xt.size();i<dfscodes.size();++i ){
            if (dfscodes[i].contain(DFS_CODE) == 0){
                for (int j=0, count=i - Xt.size(); j < count;++j )
                    Xt.push_back(vector<double>(TEST.size(), 0));
                vector<double> x(TEST.size(), 0);
                // x=subgraph_counts(projected, testSize);
                x=approximate_subgraph_counts(projected, testSize);
                for (double &xi:x)
                    xi=log(1+xi);
                Xt.push_back(x);
                break;
